...
public class FrameSenderThread extends Thread {
    ...
    private void runLockstepFrame() throws InterruptedException {
        ...
        if (buf.length != needed) {
            buf = new int[needed];
            if (!threaded.pollFrame(buf)) return;
        }

        sendLockstepAudio(server);
        sendVideoFrame(server, w, h);
    }
    ...
    private void sendLockstepAudio(MinecraftServer server) {
        int avail = runtime.getAudioAvailable();
        if (avail <= 0) return;
        if (avail > audioChunk.length) {
            audioChunk = new short[avail];
        }
        int n = runtime.readAudio(audioChunk, avail);
        dispatchAudio(server, n);
    }

    /** Interleaved stereo 16-bit LE — без downmix в mono (меньше артефактов). */
    private void dispatchAudio(MinecraftServer server, int nShorts) {
